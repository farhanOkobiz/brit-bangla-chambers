...
export default function ShowIndividualAdvocate() {
  ...
  return (
    <div className="max-w-9xl mx-auto p-8 border border-gray-200 rounded-xl bg-white shadow">
      ...
      <p><strong>Email:</strong> {advocate.user_id?.email}</p>
      <p><strong>Phone:</strong> {advocate.contact?.phone}</p>
      <p><strong>Office:</strong> {advocate.office_address}</p>
      <p><strong>Experience:</strong> {advocate.experience_years} yrs</p>
        ...
        {Array.isArray(advocate.education_ids) && advocate.education_ids.length > 0 ? (
          <ul>
            {advocate.education_ids.map((edu) => (
              <li key={edu._id}>
                <strong>{edu.degree_title}</strong> from {edu.institution} ({edu.passing_year})
              </li>
            ))}
          </ul>
        ) : (
          ...
          <ul>
            {advocate.document_ids.map((doc) => (
              <li key={doc._id}>
                ...
                {doc.file_url && (
                  <a href={doc.file_url} target="_blank" rel="noopener noreferrer" style={{ marginLeft: "8px" }}>
                    View
                  </a>
                )}
              </li>
        ...
        {Array.isArray(advocate.bar_memberships) && advocate.bar_memberships.length > 0 ? (
          <ul>
            {advocate.bar_memberships.map((bar) => (
              <li key={bar._id}>
                <strong>{bar.bar_name}</strong> (Membership #: {bar.membership_number})
              </li>
            ))}
          </ul>
        ) : (
      ...
      <div style={{ marginTop: "1.5rem" }}>
        <h3>Fee Structure</h3>
        <p><strong>Base Fee:</strong> {advocate.fee_structure?.base_fee}</p>
        <p><strong>Show Publicly:</strong> {advocate.fee_structure?.show_publicly ? "Yes" : "No"}</p>
      </div>

      {/* Stats */}
      <div style={{ marginTop: "1.5rem" }}>
        <h3>Stats</h3>
        <p><strong>Total Consultations:</strong> {advocate.stats?.total_consultations}</p>
        <p><strong>Weekly Bookings:</strong> {advocate.stats?.weekly_bookings}</p>
        {advocate.stats?.last_consultation && (
          <p><strong>Last Consultation:</strong> {new Date(advocate.stats.last_consultation).toLocaleString()}</p>
        )}
      </div>
    </div>
  );
